...
class DeepLPFLoss(nn.Module):
    ...
    def compute_msssim(self, img1, img2):
        ...
        #
        # The finest-scale SSIM term enters the product ONCE, alongside the
        # per-scale contrast terms. Writing it as `pow1[:-1] * pow2[-1]` first
        # broadcasts it into all four factors, so it would be raised to the
        # fourth power.
        #
        ...
        return pow1[0] * pow1[1] * pow1[2] * pow1[3] * pow2[-1]
